...
somewhat difficult due to the integration of both
...
The settings.py had many changes, and a few fields, although they remain
unchanged, played a role in understanding how Django handles custom
authentication strategies. Simple JWT is meant to work with Django in a more
automated fashion. However, because we are integrating it in conjunction with
Python Social Auth, using Simple JWT requires us to forego some of these
...
Note that the hash you see in our actual will need to be expored out to
`backend/.env` and a corresponding empty field provided in `backend/env-local`
in the near future.
...
`"DEFAULT_AUTHENTICATION_CLASSES"` list. This is more a precautionary measure.
...
class which holds the authentication strategy responsible for verifying the csrf
token. Also note that we provide at the end the
`rest_framework_simplejwt.authentication.JWTAuthentication`. Ultimately I
believe that these strategies are possibly NEVER implemented due to the custom
`CookieTokenAuthentication` strategy overriding all other strategies. This is
important because:
...
the `user` field being an `instanceof` `AnonymousUser`, which will be part of
one of the necessary conditional checks in our `authentication_test` route
handler which we will cover soon.
...
strategy from the `rest_framework.authentcation` library. We then declare our
`authenticate()` method, which takes an instance of `self` and `request`. Note
that this `request` is indeed the HTTP request that is passed, again, whenever
an HTTP POST with the "credentials: 'include'" header.

Then we grab the request's cookies. If the credentials we're included, then the
http only secure cookies are included in the HTTP request's headers. In this
case we grab the access token. If there is no `access_token` cookie in the
headers, then we simply return `None` (request.user is instanceof
AnonymousUser).
...
Essentially this if statement is saying "is it a JWT", if it is continue.

The SimpleJWT library then extracts the payload from the JWT using the
`AccessToken()` method. Shold this fail (i.e. the JWT is invalid/expired), an
...
is stored in a variable called `user` and as long as that user exists (i.e. is
not None), then we return a tuple containing both the user and the access_token.
...
You might be wondering, "why not include GoogleOAuth2 logic here as well?".
That's a good question and I did make attempts at putting this logic in the else
clause, but in order to use GoogleOAuth2 logic, we need to bring in the `@psa`
macro which gives us access to the `request.backend.do_auth()` method. Placing
this macro within this class caused the program to crash, as it the `@psa` macro
appears to be meant to be utilized only within route handlers, and not within
custom authentication strategies. Thusly the do_auth method (which is a strategy
in and of itself) is utilized directly within our `authentication_test()` route
handler and also within the `register_by_access_token()` and
`login_by_access_token()` route handlers which include the `@psa` macro (again,
by necessity to use the `request.backend.do_auth()` method).
...
Email and GoogleOAuth2, but Login still only works for GoogleOAuth2. Small
changes were made to the `register_by_access_token()` method, which signs up the
user using GoogleOAuth2, as well as `login_by_access_token()` method, which logs
the user in using GoogleOAuth2. Really the only thing of note is that the use of
the `rest_framework.authtoken.models import Token` was removed as it proved
unnecessary and introduced bugs, as this technically was meant to be a
SessionAuthentication strategy. I removed all use of it, and it specifically is
no longer passed as the `access_token` to the authentication cookies in both
...
Moving on, the meant of our authentication logic occurs in a series of
...
any of our client side Route components (se frontend/router/index.tsx). This
context provider component immediately makes a POST request to our backend's
`authentication_test()` route handler, and passes in its headers the
...
is present, the custom `CookieTokenAuthentication` strategy is hit. This checks
if the `access_token` is present, and is a valid JWT. If it is, it returns the
tuple with the user and the access_token, but how do we access it from within
the `authentication_test()` method?
...
`request.user` field. The GoogleOAuth2 strategy will store the google email
string in the `request.user` field. This is how we will differentiate whether
the user initially signed up with Google or not. Let's walk through the strategy
line by line.
...
`authentication_test()` route handler runs. The JWT is valid and returns our
User from the database. Then within our `authentication_test()` route, this
first check occurs after once again grabbing the `access_token` cookie from the
request.
...
field to an instanceof `AnonymousUser`. <em>However</em>, this could still mean
we signed up with GoogleOAuth2. How can we determine that? Well now we're in the
land of using Python Social Auth, right? Thusly we need the `@psa` macro. This
will then populate the request.user field with the Google User's Name, (i.e. a
string that is the name they signed up to google with). The `@psa` macro also
appends the `backend.do_auth()` method to the request dictionary, which we can
then use to "magically" authenticate the user via the access_token. Thusly, the
next conditional within our `authentication_test()` route handler looks like
this:
...
This simple elif check simply does the "magic" work provided by Python Social
Auth. This essentially reaches out to Google and checks the access_token to see
if it is still good. If it is still good, then Django/Python Social Auth reaches
into the `auth_user` table and retrieves the user should you need to use it.
Here we simply check of the `do_auth` method returns anything at all (i.e. not
None). If it doesn, then we can assume that the user successfully authenticated
using Google OAuth2.

So what next? Well in this situation, both JWT and GoogleOAuth2 strategies have
failed. Only one other condition is truly known to me at the time of this
writing, but of course, there are defensive checks and exceptions thrown should
edge cases arise during this check. The next conditional looks like this:
...
Succeeds because the previous two attempts two attempts to verify the user being
an instance of User using two diffeerent authentication strategies (one our
custom `CookieTokenAuthentication` strategy, and the other Python Social Auth's
GoogleOAuth2 strategy) failed.

In this situation we simply log a warning to our custom logger that somehow an
instance of AnonymousUser passed through our gates and we simply return a
HTTP_401_UNAUTHORIZED status back to the user. This logic on the frontend's
`<AuthContextProvider>` then throws the `if (!res.ok)` clause, setting an error
and the client side router (react-router), routes the user back to the home
landing page.
...
the previous `@permission_classes[isAuthenticated]` macro through errors, and I
believe runs its own methods which at the time of this writing I am unfamiliar
with.
...
iwth the `request.user` field, but the logic required only a simple if/elif
check, and is relatively trivial by comparison to this update to authentication.
